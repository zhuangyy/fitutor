...
**定位：** 健身好搭子 — 一个健身动作编排和跟练 App。面向有一定健身基础、知道自己该练什么、会自己编排动作，但希望有个声音在耳边指导、伴随锻炼的使用者。
...
- **Concentric border radius：** PlanCard 外层 Card radius 12→28（内层 12 + padding 16 = concentric），消除嵌套圆角平行曲线的视觉不适
- **等宽数字：** CountdownRing 倒计时数字和跟练页进度计数器添加 `FontFeature.tabularFigures()`，数字切换时不再左右跳动
- **最小触控区域：** 计划卡片拖拽手柄扩大至 40×40px，确保触控舒适
- **倒计时环定位：** 用 `Visibility(maintainSize: true)` 替代条件渲染，训练/休息阶段切换时倒计时圆环不上下跳动
- **Release 签名：** 新增正式 keystore，Release APK 不再使用 debug 签名
- **应用图标：** 全新耳机+哑铃融合图标，覆盖 Android / iOS / macOS / Web 全平台
- **应用名称：** 更名为「健身好搭子」
...
### 11.1 核心创意融合（健身 + 音乐双概念）

这是一款健身跟练类 App 的应用图标，把头戴耳机与哑铃二合一，精准传递产品核心定位：运动健身 + 健身背景音乐 / 音频课程。

**上半部分：耳机弧形头梁**

顶部暖橙铜色弧形结构，完全复刻头戴式耳机的头梁造型，直接点明「音乐、音频、运动歌单、跟练语音指导」的音频属性；暖橙金属质感，视觉活力、契合运动热血氛围。

**下半部分：哑铃替代耳机耳罩**

左右两个哑铃取代耳机耳罩，中间金属哑铃杆贯穿两侧，完美对应健身主题；哑铃本身是力量训练标志性器械，一眼就能看懂健身赛道定位。

### 11.2 视觉结构与细节

- 左右哑铃各分层金属杠铃片，每侧搭配 1 条橙色装饰环，和上方橙色头梁色彩呼应
- 金属灰片自带同心圆分层纹理、中心圆孔，还原真实杠铃片工业质感，层次清晰
- 缩小到手机桌面小图标时轮廓依然辨识度很高

### 11.3 色彩搭配

| 元素 | 颜色 | 说明 |
|------|------|------|
| 背景 | 深海军蓝渐变圆角方形 | 深色沉稳耐看，适配浅色/深色系统桌面 |
| 头梁 + 哑铃装饰条 | 橙铜色 | 暖色金属质感，运动活力 |
| 杠铃杆、配重片 | 冷金属银灰 | 工业质感，冷暖撞色对比强烈 |

### 11.4 风格质感

轻拟物扁平化结合，有柔和光影、微弱立体浮雕层次，没有复杂冗余细节，兼顾精致感与图标小尺寸识别性，符合主流 App 图标设计规范。

### 11.5 整体寓意

深蓝色底色代表专注自律，橙色代表活力运动，耳机 + 哑铃的组合直观传递产品功能：一边提供健身训练内容，一边配套运动音频、跟练背景音乐，用户看到图标就能快速理解 App 是用来边听音乐边健身训练。
